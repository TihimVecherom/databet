(() => {
    ...
    if (menuBurger) menuBurger.addEventListener("click", (function(e) {
        ...
        if (body) body.classList.add("_body-active-bg");
        if (userMenu) userMenu.classList.remove("_user-menu-active");
        if (maiN) maiN.classList.remove("_main-bg-active");
    }));
    const script_menuClose = document.querySelector(".menu__btn-close");
    if (script_menuClose) script_menuClose.addEventListener("click", (function(e) {
        if (menuBody) menuBody.classList.remove("_menu-active");
        if (body) body.classList.remove("_body-active-bg");
        if (body) body.classList.remove("_hidden-active");
    }));
    const btnUserMenu = document.querySelector(".user-btn");
    const userMenu = document.querySelector(".user-menu");
    const maiN = document.querySelector(".page");
    if (btnUserMenu) btnUserMenu.addEventListener("click", (function(e) {
        if (window.innerWidth < 991) {
            if (userMenu) userMenu.classList.toggle("_user-menu-active");
            if (maiN) maiN.classList.toggle("_main-bg-active");
            if (document.body) document.body.classList.toggle("_hidden-active");
        }
    }));
    ...
    const btnTab = document.querySelector(".toggle-btn");
    const menuTab = document.querySelector(".examples__content");
    if (window.innerWidth < 767.98) if (btnTab) btnTab.addEventListener("click", (function(e) {
        btnTab.classList.toggle("_active-tab-btn");
        if (menuTab) menuTab.classList.toggle("_active-tab");
    }));
    const btnInfo = document.querySelector(".questions__info-i");
    const menuOnfo = document.querySelector(".questions__content-wrap");
    if (window.innerWidth < 991.98) if (btnInfo) btnInfo.addEventListener("click", (function(e) {
        btnInfo.classList.toggle("_active-info-btn");
        if (menuOnfo) menuOnfo.classList.toggle("_active-info-menu");
    }));
    const btn = document.querySelector(".genarate-btn");
    const contentBlock = document.getElementById("contentBlock");
    const anotherBtn = document.querySelector(".data-package__bar-btn");
    if (btn && contentBlock && anotherBtn) btn.addEventListener("click", (function() {
        const progressText = btn.querySelector(".genarate-btn-progress");
        let width = 0;
        let intervalId;
        ...
        function frame() {
            if (width >= 100) {
                ...
                anotherBtn.classList.remove("disabled");
            } else {
                width++;
    ...
    let script_elements = document.querySelectorAll(".popup__form-input");
    script_elements.forEach(((item, index) => {
        item.addEventListener("input", (function() {
            if (item.value.length) item.classList.add("_valid-btn"); else item.classList.remove("_valid-btn");
        }));
    }));
    let passInputs = document.querySelectorAll(".password-check");
    passInputs.forEach((function(passInput) {
        passInput.addEventListener("input", (function() {
            let passLength = this.value.length;
            if (0 === passLength) this.classList.remove("error", "match"); else if (passLength < 6) {
        ...
        let passBtns = document.querySelectorAll(".button-show-password");
        passBtns.forEach((function(btn) {
            btn.onclick = function() {
                let target = this.getAttribute("data-target");
                let passInput = document.querySelector(target);
                if (passInput && "password" === passInput.getAttribute("type")) passInput.setAttribute("type", "text"); else if (passInput) passInput.setAttribute("type", "password");
            };
        }));
    }));
    ...
    const btnPromo = document.querySelector(".btn-promocode");
    const menuPromo = document.querySelector(".promocode__content");
    if (btnPromo) btnPromo.addEventListener("click", (function(e) {
        btnPromo.classList.toggle("_active");
        if (menuPromo) menuPromo.classList.toggle("_active");
    }));
    window["FLS"] = true;
    isWebp();
    spollers();
